# FROZEN_COPY_V2 (2026-06-25): AG_src/pipeline/step08_stability.py 검증 기준선 사본.
# 원본은 AG_src/pipeline/step08_stability.py (동결).
...
_MODIFICATION_LOG_MULT: Dict[str, float] = {
    ...
    "d_amino_acid":  math.log10(35.0),    # (레거시 단일값, 아래 _D_AA_PER_RESIDUE_LOG 참조)
    "cyclization":   math.log10(1.8),     # 이황화/고리화: 완만한 안정화 (혈중 t½ 주도 아님)
    "substitution":  math.log10(4.0),
}

# ── 2026-06-25 D-aa 잔기수 비례 계수 (정확도 개선) ──────────────────────────
# ⚠️ HEURISTIC: 아래 계수는 문헌 kcat/Km 데이터 부재 하에 추정된 값이다.
#   근거:
#     Bivalirudin (D-Phe1 1개, IV 혈중 t½≈0.42h): ×35 일괄 적용 시 1.18h(과대).
#       기본값 0.034h → 목표 0.42h 달성 요구 ×12.4 → ×10 채택 (예측 0.34h, 오차 ~19%).
#     단일 D-aa는 해당 절단부위 1개만 보호 → ×10 (log10(10.0)=1.0).
#     2개 이상: 기존 ×35 cap 유지 (D-aa 밀도 높을수록 포화).
#     잔기수 비례: log_mult = per_residue × n_d_aa, cap=log10(35).
#   TODO: 실측 in-vitro 혈청 어세이 데이터로 계수 교체 권장.
_D_AA_PER_RESIDUE_LOG: float = math.log10(10.0)  # HEURISTIC: D-aa 1개당 log_mult (Bivalirudin 역산)
_D_AA_CAP_LOG: float = math.log10(35.0)           # HEURISTIC: 전체 치환 상한 (기존값 보존)


def _count_d_aa(sequence: str, modifications: List[str]) -> int:
    """D-aa 잔기 수를 추정한다.

    우선순위:
      1) modifications에 'D_aa:N' 또는 'd_amino_acid:N' 형태가 있으면 N 사용.
      2) 서열에 소문자 토큰이 있으면 소문자 개수 사용 (소문자=D-aa 관행).
      3) modifications에 'd_amino_acid'만 있으면 1 반환 (단일 치환 기본값).

    Args:
        sequence:      원본 전달 서열 (대소문자 포함 가능).
        modifications: modification 유형 목록.

    Returns:
        추정 D-aa 잔기 수 (0 이상).
    """
    # 1) 'd_amino_acid:N' 또는 'D_aa:N' 형식 파싱
    for mod in modifications:
        m_lower = str(mod).lower()
        if ("d_amino_acid:" in m_lower or "d_aa:" in m_lower):
            try:
                n = int(m_lower.split(":")[-1])
                return max(0, n)
            except ValueError:
                pass

    # 2) 서열 소문자 토큰 (D-aa 소문자 관행)
    n_lower = sum(1 for c in sequence if c.islower() and c.upper() in _PROTEASE_VULNERABILITY)
    if n_lower > 0:
        return n_lower

    # 3) 'd_amino_acid' 단순 항목 → 기본값 1
    has_d = any(
        "d_amino" in str(m).lower() or "d-amino" in str(m).lower()
        for m in modifications
    )
    return 1 if has_d else 0


def _d_aa_log_mult(n_d_aa: int) -> float:
    """D-aa 잔기 수 n_d_aa에 대한 log_mult를 계산한다.

    # HEURISTIC: log10(3.0) per residue, cap=log10(35)
    """
    if n_d_aa <= 0:
        return 0.0
    return min(_D_AA_CAP_LOG, _D_AA_PER_RESIDUE_LOG * n_d_aa)
...
def predict_half_life(
    ...
    GLP-1 약동학 모델 기반 반감기 예측:
      ...
      4) 적용된 modification 보너스 합산 (D-aa는 잔기수 비례)

    Args:
        sequence:      아미노산 서열 (1-letter code, 대/소문자 혼용 가능;
                       소문자=D-aa 관행일 때 개수 자동 감지)
        modifications: 이미 적용된 modification 유형 목록
                       예: ["fatty_acid", "d_amino_acid", "cyclization"]
                       D-aa 개수 힌트: "d_amino_acid:3" 형식도 수용.
    ...
    #    D-aa는 잔기수 비례 계수 사용 (2026-06-25 개선).
    #    ⚠️ HEURISTIC: D-aa 비례 계수는 추정값 — _d_aa_log_mult 주석 참조.
    has_d_amino = False
    for mod in modifications:
        m_lower = str(mod).lower()
        if "d_amino" in m_lower or "d-amino" in m_lower:
            has_d_amino = True  # D-aa는 별도 처리
        elif m_lower in _MODIFICATION_LOG_MULT:
            mod_log += _MODIFICATION_LOG_MULT[m_lower]
        elif "fatty" in m_lower or "acyl" in m_lower:
            mod_log += _MODIFICATION_LOG_MULT["fatty_acid"]
        elif "peg" in m_lower:
            mod_log += _MODIFICATION_LOG_MULT["pegylation"]
        elif m_lower in ("cyclization", "substitution"):
            mod_log += _MODIFICATION_LOG_MULT[m_lower]

    # D-aa: 잔기수 비례 log_mult (단일 치환 vs 전체 치환 구분)
    if has_d_amino:
        n_d = _count_d_aa(sequence, modifications)
        mod_log += _d_aa_log_mult(n_d)
        logger.debug(
            "D-aa 잔기수 비례 적용: n_d=%d, log_mult=%.4f (HEURISTIC)",
            n_d, _d_aa_log_mult(n_d),
        )
