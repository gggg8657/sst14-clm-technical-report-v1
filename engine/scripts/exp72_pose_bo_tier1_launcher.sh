#!/usr/bin/env bash
# Pose BO 프로토 검증 통과 후 Tier 1 18종 자동 착수 launcher.
# 프로토 완료(10 iter) 대기 → best_f 검증(>0) → Tier 1 8샤드 세션독립 기동.
# 정지 = touch _workspace/STOP_POSE_BO_TIER1
set -u
REPO="$(cd "$(dirname "$0")/.." && pwd)"; cd "$REPO"
BIO=/home/dongjukim/miniforge3/envs/bio-tools/bin/python
LOG=runs/exp72_analysis/pose_bo_tier1.log
PROTO=runs/exp72_analysis/pose_bo_proto/AGCKNFFWKTFTSC_bo_history.jsonl
PIN="OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 NUMEXPR_NUM_THREADS=1"
SHARDS=8
say(){ echo "$(date -u +%FT%TZ) $*" >> "$LOG"; }

mkdir -p runs/exp72_analysis/pose_bo runs/exp72_analysis/pose_bo_logs
say "TIER1 launcher start — 프로토(10 iter) 대기"
...
# 정상성 판정
OK=$($BIO -c "print(1 if float('$BF')>0 and int('$ERR')<8 else 0)")
if [ "$OK" != "1" ]; then
...
say "✅ 프로토 통과 → Tier 1 18종 8샤드 착수"
for i in $(seq 0 $((SHARDS-1))); do
  setsid nice -n 5 env $PIN "$BIO" scripts/exp72_pose_bo.py \
    --shard-index "$i" --shard-count "$SHARDS" \
    --n-iter 100 --n-initial 10 \
    --work-dir runs/exp72_analysis/pose_bo \
    > "runs/exp72_analysis/pose_bo_logs/shard_${i}.log" 2>&1 < /dev/null & sleep 0.5
done
say "Tier 1 $SHARDS 샤드 기동 완료"

# 완료 대기 = summary 18 records
while true; do
  [ -f _workspace/STOP_POSE_BO_TIER1 ] && { say "STOP 요청 종료"; exit 0; }
  n=$(wc -l < runs/exp72_analysis/pose_bo/bo_summary.jsonl 2>/dev/null || echo 0)
  [ "$n" -ge 18 ] && break
  sleep 600
done
say "TIER1 DONE — $n / 18"
